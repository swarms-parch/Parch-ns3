...
// Cryptographic CPU execution time is not advanced by Simulator::Now(); separate Raspberry Pi 5
// benchmarks remain the source of primitive computation times.
...
struct PerformanceMetrics
{
    ...
    Time authEnd{Seconds(0)};
    uint64_t authRxBytes{0};
    uint32_t authRxPackets{0};
    bool authActive{false};

    Time handoverStart{Seconds(0)};
    Time handoverEnd{Seconds(0)};
    uint64_t handoverRxBytes{0};
    uint32_t handoverRxPackets{0};
...
class ParchApp : public Application
{
  public:
    ...
    void StartInitialAuthentication()
    {
        ...
        auto& metrics = m_system->metrics;
        metrics.authStart = Simulator::Now();
        metrics.authEnd = Seconds(0);
        metrics.authRxBytes = 0;
        metrics.authRxPackets = 0;
        metrics.authActive = true;
    ...
    void StartHandover()
    {
        ...
        auto& metrics = m_system->metrics;
        metrics.handoverStart = Simulator::Now();
        metrics.handoverEnd = Seconds(0);
        metrics.handoverRxBytes = 0;
        metrics.handoverRxPackets = 0;
        metrics.handoverActive = true;
  ...
  private:
    ...
    void RecordAcceptedPacket(MessageType type, size_t bytes)
    {
        auto& metrics = m_system->metrics;

        if (type == AUTH_M0 || type == AUTH_M1 || type == AUTH_M2)
        {
            if (metrics.authActive)
            {
                metrics.authRxBytes += static_cast<uint64_t>(bytes);
                ++metrics.authRxPackets;
            }
        }
        else if (type == HO_M3 || type == HO_M4)
        {
            if (metrics.handoverActive)
            {
                metrics.handoverRxBytes += static_cast<uint64_t>(bytes);
                ++metrics.handoverRxPackets;
            }
        }
    }

    static void PrintPerformance(const std::string& name,
                                 Time start,
                                 Time end,
                                 uint64_t bytes,
                                 uint32_t packets)
    {
        ...
        double latencyMs = elapsed.GetSeconds() * 1000.0;
        double throughputMbps = 0.0;

        if (elapsed.GetSeconds() > 0.0)
        {
            throughputMbps =
                (static_cast<double>(bytes) * 8.0) /
                elapsed.GetSeconds() / 1e6;
        }
        ...
        std::cout << "\n=== " << name << " PERFORMANCE ===\n"
                  ...
                  << latencyMs << " ms\n"
                  << "Accepted protocol packets: "
                  << packets << "\n"
                  << "Accepted protocol bytes  : "
                  << bytes << " bytes\n"
    ...
    void HandleAuthM0(const Bytes& data)
    {
        ...
        RecordAcceptedPacket(AUTH_M0, data.size());
    ...
    void HandleAuthM1(const Bytes& data)
    {
        ...
        RecordAcceptedPacket(AUTH_M1, data.size());
    ...
    void HandleAuthM2(const Bytes& data)
    {
        ...
        RecordAcceptedPacket(AUTH_M2, data.size());
        auto& metrics = m_system->metrics;
        metrics.authEnd = Simulator::Now();
        metrics.authActive = false;
        ...
        PrintPerformance("GCS-CH AUTHENTICATION",
                         metrics.authStart,
                         metrics.authEnd,
                         metrics.authRxBytes,
                         metrics.authRxPackets);
    ...
    void HandleHandoverM3(const Bytes& data)
    {
        ...
        RecordAcceptedPacket(HO_M3, data.size());
    ...
    void HandleHandoverM4(const Bytes& data)
    {
        ...
        RecordAcceptedPacket(HO_M4, data.size());
        auto& metrics = m_system->metrics;
        metrics.handoverEnd = Simulator::Now();
        metrics.handoverActive = false;
        ...
        PrintPerformance("FAILURE-RESILIENT HANDOVER",
                         metrics.handoverStart,
                         metrics.handoverEnd,
                         metrics.handoverRxBytes,
                         metrics.handoverRxPackets);
    }
...
int main(int argc, char* argv[])
{
    ...
    std::cout << "=== PARCH ns-3 functional validation, latency and throughput evaluation ===\n"
              << "GCS     : " << gcsAddr << "\n"
              << "CH      : " << chAddr << "\n"
              << "ACC_old : " << oldAddr << "\n"
              << "ACC_new : " << newAddr << "\n"
